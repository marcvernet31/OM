%Modificació de l'algoritme per compraovar a cada iteració que d(k) és 
%perpendicular a d(k-1).
%   El resultat es pot veure a l'íltima columna dels valors que surten per
%   la consola
% Amb la funció de CE 2.1 en la majoria d'iteracions no es compleix.
...
epsG = 10^-6; kmax = 1500;
 % Linesearch:
almax = 2; almin = 10^-3; rho=0.5; c1=0.01; c2=0.5; iW = 0;
 % Search direction:
...
% Optimization
[xk,dk,alk,iWk,betak,Hk, check_p] = om_uo_solve_mod(x,f,g,h,epsG,kmax,almax,almin,rho,c1,c2,iW,isd,icg,irc,nu);
...
fprintf('k x(1) x(2) iW g''*d ||g|| r check_p\n');
for k = 1:niter-1
    fprintf('%5d %7.4f %7.4f %3d %+3.1e %4.2e %3.1e %3.0e\n', k, xk(1,k), xk(2,k), iWk(k), gdk(k), norm(gk(:,k)), rk(k), check_p(k));
end
